...
let
  ...
  makeTest =
    ...
    let
      ...
      pkgs = nixpkgsFor.${image.system}.native;
    in
    pkgs.runCommand "installer-test-${imageName}-${testName}"
      {
        buildInputs = [
          pkgs.qemu_kvm
          pkgs.openssh
        ...
        installScript = testScripts.${testName}.script;
        binaryTarball = binaryTarballs.${pkgs.system};
      }
      ''
        shopt -s nullglob
...
builtins.mapAttrs (imageName: image: {
  ${image.system} = builtins.mapAttrs (testName: test: makeTest imageName testName) testScripts;
}) images
